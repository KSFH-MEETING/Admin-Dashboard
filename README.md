...
Dashboard → Inventory manages a small stock of meeting equipment with short IDs such as `EQ-001`. Each item supports a category, brand, model, optional serial number, acquired date, warranty expiry, condition, responsible person or department, specification, and a storage location selected from the meeting rooms, the I.T equipment room, or a custom value. A serialized item is tracked individually with a total quantity of one. Availability is calculated from total, reserved, in-use, and damaged quantities. Inventory changes are stored as append-only rows in the `Inventory` tab of the configured Google Sheet. Owner can add, edit, or disable items; Editor and Viewer have read-only access; Guest cannot open Inventory.
