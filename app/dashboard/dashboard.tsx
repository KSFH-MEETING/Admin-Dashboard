    ...
    <main className="min-h-screen bg-slate-50">
      <header className="border-b bg-[#075d45] text-white print:hidden">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          <div className="grid w-full grid-cols-2 items-center gap-2 sm:flex sm:w-auto sm:flex-wrap sm:justify-end"><div className="col-span-2 text-left text-xs sm:text-right"><p className="font-semibold">{guest ? 'Guest' : user.name || email}</p>{!guest && <p className="max-w-56 truncate text-emerald-100" title={email}>{email}</p>}<p className="mt-1 text-emerald-100">{guest ? '🌐 សាធារណៈ · មើល Booking តែប៉ុណ្ណោះ' : user.role === 'owner' ? '👑 ម្ចាស់ប្រព័ន្ធ' : user.role === 'editor' ? '✏️ គ្រប់គ្រង Booking' : '👁️ មើលតែប៉ុណ្ណោះ'}</p></div><Button className="h-10 w-full sm:w-auto" variant="secondary" disabled={signingOut} onClick={onSignOut}>{signingOut ? 'កំពុងចាកចេញ…' : guest ? 'ចូលគណនី Google' : 'ចាកចេញ'}</Button>{!guest && <Link className="block" href="/request" target="_blank"><Button className="h-10 w-full sm:w-auto" variant="secondary"><Plus /> Form ស្នើសុំ <ExternalLink className="size-3" /></Button></Link>}</div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        <section id="booking-list" aria-labelledby="booking-list-title" className="rounded-2xl border bg-white shadow-sm">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-slate-50 px-4 py-3"><p className="text-xs text-slate-600">ម៉ោងកម្ពុជា · ចុចចំណងជើងការកក់ ដើម្បីមើលព័ត៌មានលម្អិត។</p><div className="flex flex-wrap gap-2"><Button variant="ghost" onClick={() => { setPeriod('all'); setStatus('ACTIVE'); setQuery(''); setPage(1); }}>សម្អាតតម្រង</Button>{!guest && <a href="/request" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-800"><Plus className="size-4" />កក់បន្ទប់ថ្មី<ExternalLink className="size-3" /></a>}</div></div>
