...
async function initializeInventory() {
  ...
  await googleFetch(
    `${api()}/values/${encodeURIComponent(`'${INVENTORY_SHEET}'!A1:U1`)}?valueInputOption=RAW`,
    {
      method: 'PUT',
...
export async function listInventoryItems() {
  ...
  const response = await googleFetch(
    `${api()}/values/${encodeURIComponent(`'${INVENTORY_SHEET}'!A2:U`)}`,
  );
...
export async function saveInventoryItem(
  ...
  await googleFetch(
    `${api()}/values/${encodeURIComponent(`'${INVENTORY_SHEET}'!A:U`)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
    {
      method: 'POST',
      body: JSON.stringify({
        values: [
